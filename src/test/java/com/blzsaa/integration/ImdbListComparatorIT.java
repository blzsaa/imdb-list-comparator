...
public class ImdbListComparatorIT {
    ...
    public void canUploadCsv() throws Exception {
        ...
        ResultActions resultActions = mvc.perform(
                fileUpload("/uploadList").file(m1).param("list", "movies1").sessionAttr("movies", new HashMap<>()));

        // then
        resultActions.andExpect(content().string(containsString("Title1")))
                .andExpect(content().string(containsString("Title2")))
                .andExpect(model().attribute("movies", ImmutableMap.of("movies1", asList("Title1", "Title2"))))
    ...
    public void canUploadSecondCsv() throws Exception {
        ...
        // when
        ResultActions resultActions = mvc.perform(fileUpload("/uploadList").file(m1).param("list", "movies2")
        ...
        resultActions.andExpect(content().string(containsString("Title1")))
                .andExpect(content().string(containsString("Title2")))
                .andExpect(model().attribute("movies", expectedMap)).andExpect(status().isOk());
    }
